...
export default function InvoiceResultPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/search-invoice?${query}`);
  ...
  return (
    <div className="max-w-full mx-auto mt-10 p-4">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          onClick={() =>
            navigate(memberType === "MEMBER" ? "/invoice-member" : "/invoice-nonmember")
          }
                ...
                return (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-1 border">
                      {row.e_bill_file_id ? (
                        <a
                          href={`${FILE_SERVICE_URL}${row.e_bill_file_id}?key=${API_KEY}`}
                          target="_blank"
